...
<script type="text/javascript">
	...
	function changeState(roomId,timeId,name,state){
        ...
        if (state==0){
        	...
        	targetAObject.setAttribute("onclick","addFilm("+roomId+","+timeId+")");
		...
		if (state==1){
			//修改a中的链接
        	var targetAObject=document.getElementById("a"+roomId+"+"+timeId);
        	targetAObject.setAttribute("onclick","ajax_removePlan("+roomId+","+timeId+")");
		...
		if (state==2){
        	...
        	//修改图标
        	var targetIObject=document.getElementById("i"+roomId+"+"+timeId);
        	targetIObject.className="icon-ok";
		}
	}
</script>
</head>
...
<body onload="init_makePlan()">
	...
	<div class="row" style="margin-top:80px">
		...
		<div class="col-md-offset-4 col-md-4">
			<label>日期：</label> <input id="playTime" type="text" value="${vo.dateString}" name="HBegtime"
						...
						<c:forEach var="times" items="${vo.timequantums}">
							<td id="td${rooms.roomId}+${times.id}"><c:if
								...
								onclick="addFilm(${rooms.roomId},${times.id})"> <i
									...
									<span id="${rooms.roomId}+${times.id}">添加电影</span>
							</a></td>
						</c:forEach>
					</tr>
				</c:forEach>
